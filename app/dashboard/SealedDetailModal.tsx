"use client";

import { useCallback, useEffect, useMemo, useState } from "react";
import { Check, ChevronDown, DollarSign, Layers, LoaderCircle, Package, Plus, Trash2 } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogDescription,
} from "@/components/ui/dialog";
import { Skeleton } from "@/components/ui/skeleton";
import {
  Table,
  TableBody,
  TableCell,
  TableHead,
  TableHeader,
  TableRow,
} from "@/components/ui/table";
import { Button } from "@/components/ui/button";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuRadioGroup,
  DropdownMenuRadioItem,
  DropdownMenuTrigger,
} from "@/components/ui/dropdown-menu";
import { createClient } from "@/lib/supabase/client";
import { useTranslation } from "@/lib/i18n";
import { AddToLotPopover } from "./AddToLotPopover";
import { useCurrency } from "./CurrencyContext";
import { useBuyList } from "./BuyListContext";
import { Badge } from "@/components/ui/badge";
import { Input } from "@/components/ui/input";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from "@/components/ui/popover";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
  AlertDialogContent,
  AlertDialogDescription,
  AlertDialogFooter,
  AlertDialogHeader,
  AlertDialogTitle,
  AlertDialogTrigger,
} from "@/components/ui/alert-dialog";
import {
  type LocationInfo,
  fetchRateMap,
  fetchLocationMap,
  getCardDisplayName,
} from "./use-card-data";
import {
  type SealedRowData,
  conditionLabel,
  editionLabel,
  productTypeLabel,
} from "./use-sealed-data";
import { useLanguage } from "./LanguageContext";

const SEALED_ENTRY_TABLE = "pokemon_sealed_buylist_entries";

interface SealedListing {
  price: number;
  currency: string;
  currencySymbol: string;
  sealedCondition: string;
  variantEdition: string;
  locationId: number;
  listingUrl: string | null;
  priceType: "Buy" | "Sell";
}

interface DetailListing {
  price: number;
  currencySymbol: string;
  currencyCode: string;
  locationName: string;
  marketRegion: string | null;
  conditionLabel: string;
  listingUrl: string | null;
}

interface SealedDetailModalProps {
  card: SealedRowData | null;
  open: boolean;
  onClose: () => void;
  onRemoveFromBuylist?: () => Promise<void> | void;
  entryId?: number;
  targetPriceUsd?: number | null;
  onTargetPriceChange?: (entryId: number, price: number | null) => void;
}

export default function SealedDetailModal({
  card,
  open,
  onClose,
  onRemoveFromBuylist,
  entryId,
  targetPriceUsd,
  onTargetPriceChange,
}: SealedDetailModalProps) {
  const { t } = useTranslation();
  const { language } = useLanguage();
  const { buylists, addToBuylist } = useBuyList();
  const [addedTo, setAddedTo] = useState<string | null>(null);
  const [rawListings, setRawListings] = useState<SealedListing[]>([]);
  const [rateMap, setRateMap] = useState<Map<string, number>>(new Map());
  const [locationMap, setLocationMap] = useState<Map<number, LocationInfo>>(
    new Map()
  );
  const [loading, setLoading] = useState(false);
  // "all" = every variant of the product; otherwise pin one enum value.
  const [condition, setCondition] = useState<string>("all");
  const [edition, setEdition] = useState<string>("all");
  const [targetPrice, setTargetPrice] = useState<string>("");
  const [savingTargetPrice, setSavingTargetPrice] = useState(false);

  const saveTargetPrice = useCallback(async () => {
    if (entryId == null || savingTargetPrice) return;
    const parsed = targetPrice === "" ? null : Number(targetPrice);
    if (parsed != null && (isNaN(parsed) || parsed < 0)) return;
    setSavingTargetPrice(true);
    const supabase = createClient();
    const { error } = await supabase
      .from(SEALED_ENTRY_TABLE)
      .update({ target_price_usd: parsed })
      .eq("entry_id", entryId);
    if (error) {
      console.error("Failed to save target price:", error);
    } else {
      onTargetPriceChange?.(entryId, parsed);
    }
    setSavingTargetPrice(false);
  }, [entryId, targetPrice, savingTargetPrice, onTargetPriceChange]);

  // Default the variant filters to the row the user clicked.
  useEffect(() => {
    if (open && card) {
      setCondition(card.sealedCondition);
      setEdition(card.variantEdition);
      setAddedTo(null);
      setTargetPrice(targetPriceUsd != null ? String(targetPriceUsd) : "");
    }
  }, [open, card, targetPriceUsd]);

  useEffect(() => {
    if (!card || !open) return;

    let cancelled = false;
    setLoading(true);

    async function fetchListings() {
      const supabase = createClient();
      const [{ data: raw }, rates, locations] = await Promise.all([
        supabase
          .from("pokemon_sealed_market_listings")
          .select(
            "product_id, price_type, price, currency, sealed_condition, variant_edition, location_id, listing_url, currencies(symbol)"
          )
          .eq("product_id", card!.card.card_id),
        fetchRateMap(supabase),
        fetchLocationMap(supabase),
      ]);

      if (cancelled) return;

      const listings: SealedListing[] = (raw ?? []).map(
        (l: Record<string, unknown>) => ({
          price: l.price as number,
          currency: l.currency as string,
          currencySymbol:
            (l.currencies as { symbol: string } | null)?.symbol ?? "",
          sealedCondition: l.sealed_condition as string,
          variantEdition: l.variant_edition as string,
          locationId: l.location_id as number,
          listingUrl: (l.listing_url as string | null) ?? null,
          priceType: l.price_type as "Buy" | "Sell",
        })
      );

      setRawListings(listings);
      setRateMap(rates);
      setLocationMap(locations);
      setLoading(false);
    }

    fetchListings();
    return () => {
      cancelled = true;
    };
  }, [card, open]);

  const { buy, sell } = useMemo(() => {
    const normalize = (l: SealedListing) =>
      l.price * (rateMap.get(l.currency) ?? 1);

    const toDetail = (l: SealedListing): DetailListing => {
      const loc = locationMap.get(l.locationId);
      return {
        price: l.price,
        currencySymbol: l.currencySymbol,
        currencyCode: l.currency,
        locationName: loc?.name ?? "",
        marketRegion: loc?.marketRegion ?? null,
        conditionLabel: `${editionLabel(t, l.variantEdition)} · ${conditionLabel(t, l.sealedCondition)}`,
        listingUrl: l.listingUrl,
      };
    };

    const matches = rawListings.filter(
      (l) =>
        (condition === "all" || l.sealedCondition === condition) &&
        (edition === "all" || l.variantEdition === edition)
    );

    const sortBuy = (a: SealedListing, b: SealedListing) =>
      normalize(b) - normalize(a);
    const sortSell = (a: SealedListing, b: SealedListing) =>
      normalize(a) - normalize(b);

    return {
      buy: matches.filter((l) => l.priceType === "Buy").sort(sortBuy).map(toDetail),
      sell: matches.filter((l) => l.priceType === "Sell").sort(sortSell).map(toDetail),
    };
  }, [rawListings, rateMap, locationMap, condition, edition, t]);

  if (!card) return null;

  const { card: def } = card;
  const setCode = def.set_code && def.set_code !== "UNKNOWN" ? def.set_code : null;
  const misc = def.misc_info && def.misc_info !== "UNKNOWN" ? def.misc_info : null;

  const CONDITION_OPTIONS = ["all", "shrink", "no_shrink", "standard"];
  const EDITION_OPTIONS = ["all", "1ed", "unlimited", "standard"];

  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-3xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex gap-4">
            {def.image_url && (
              <img
                src={def.image_url}
                alt={getCardDisplayName(def, language)}
                className="h-64 w-auto rounded-md object-contain shrink-0"
              />
            )}
            <div className="flex min-w-0 flex-col gap-1">
              <DialogTitle className="text-lg">{getCardDisplayName(def, language)}</DialogTitle>
              {misc && (
                <DialogDescription className="text-xs">{misc}</DialogDescription>
              )}
              <div className="mt-2 flex flex-wrap gap-1">
                <Badge variant="secondary" className="h-auto px-1.5 py-px">
                  <Package className="size-3" />
                  {productTypeLabel(t, card.productType)}
                </Badge>
                {setCode && (
                  <Badge variant="secondary" className="h-auto px-1.5 py-px">
                    <Layers className="size-3" />
                    {setCode}
                  </Badge>
                )}
              </div>
            </div>
          </div>
        </DialogHeader>

        {loading ? (
          <div className="grid grid-cols-2 gap-4 pt-2">
            {Array.from({ length: 2 }).map((_, i) => (
              <div key={i} className="space-y-2">
                <Skeleton className="h-4 w-16" />
                <div className="rounded-md border p-2 space-y-2">
                  {Array.from({ length: 3 }).map((_, j) => (
                    <Skeleton key={j} className="h-4 w-full" />
                  ))}
                </div>
              </div>
            ))}
          </div>
        ) : (
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              <DropdownMenu>
                <DropdownMenuTrigger render={<Button variant="outline" />}>
                  {t("sealedBrowser.conditionPrefix")}
                  {condition === "all" ? t("sealedBrowser.conditionAll") : conditionLabel(t, condition)}
                  <ChevronDown className="ml-1 size-4" />
                </DropdownMenuTrigger>
                <DropdownMenuContent>
                  <DropdownMenuRadioGroup value={condition} onValueChange={setCondition}>
                    {CONDITION_OPTIONS.map((c) => (
                      <DropdownMenuRadioItem key={c} value={c}>
                        {c === "all" ? t("sealedBrowser.conditionAll") : conditionLabel(t, c)}
                      </DropdownMenuRadioItem>
                    ))}
                  </DropdownMenuRadioGroup>
                </DropdownMenuContent>
              </DropdownMenu>
              <DropdownMenu>
                <DropdownMenuTrigger render={<Button variant="outline" />}>
                  {t("sealedBrowser.editionPrefix")}
                  {edition === "all" ? t("sealedBrowser.editionAll") : editionLabel(t, edition)}
                  <ChevronDown className="ml-1 size-4" />
                </DropdownMenuTrigger>
                <DropdownMenuContent>
                  <DropdownMenuRadioGroup value={edition} onValueChange={setEdition}>
                    {EDITION_OPTIONS.map((e) => (
                      <DropdownMenuRadioItem key={e} value={e}>
                        {e === "all" ? t("sealedBrowser.editionAll") : editionLabel(t, e)}
                      </DropdownMenuRadioItem>
                    ))}
                  </DropdownMenuRadioGroup>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <h3 className="text-sm font-medium mb-2">{t("modal.sell")}</h3>
                <ListingTable listings={sell} conditionHeader={t("modal.condition")} t={t} />
              </div>
              <div>
                <h3 className="text-sm font-medium mb-2">{t("modal.buy")}</h3>
                <ListingTable listings={buy} conditionHeader={t("modal.condition")} t={t} />
              </div>
            </div>
          </div>
        )}

        {(buylists.length > 0 || onRemoveFromBuylist) && (
          <div className="flex items-center justify-end gap-3 border-t pt-4">
            {onRemoveFromBuylist && entryId != null && (
              <div className="mr-auto flex items-center gap-2">
                <label className="text-sm text-muted-foreground whitespace-nowrap">
                  {t("buyList.targetPrice")}
                </label>
                <div className="relative">
                  <DollarSign className="absolute left-2 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" />
                  <Input
                    type="number"
                    step="0.01"
                    min="0"
                    className="w-28 pl-7"
                    value={targetPrice}
                    onChange={(e) => setTargetPrice(e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                        saveTargetPrice();
                      }
                    }}
                  />
                </div>
                <Button
                  size="icon"
                  className="size-8"
                  disabled={savingTargetPrice}
                  onClick={saveTargetPrice}
                >
                  {savingTargetPrice ? (
                    <LoaderCircle className="size-4 animate-spin" />
                  ) : (
                    <Check className="size-4" />
                  )}
                </Button>
              </div>
            )}
            {addedTo && (
              <span className="flex items-center gap-1 text-sm text-muted-foreground">
                <Check className="size-4" />
                {t("buyList.added", { name: addedTo })}
              </span>
            )}
            {onRemoveFromBuylist && (
              <AlertDialog>
                <AlertDialogTrigger render={<Button variant="outline" />}>
                  <Trash2 className="size-4" />
                  {t("buyList.removeFrom")}
                </AlertDialogTrigger>
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>{t("buyList.removeConfirm")}</AlertDialogTitle>
                    <AlertDialogDescription>{t("buyList.removeConfirmDesc")}</AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel>{t("buyList.cancel")}</AlertDialogCancel>
                    <AlertDialogAction
                      onClick={async () => {
                        await onRemoveFromBuylist();
                        onClose();
                      }}
                    >
                      {t("buyList.remove")}
                    </AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
              </AlertDialog>
            )}
            {buylists.length > 0 && (
              <Popover>
                <PopoverTrigger render={<Button />}>
                  <Plus className="size-4" />
                  {t("buyList.addTo")}
                </PopoverTrigger>
                <PopoverContent className="w-48 p-1" align="end">
                  {buylists.map((bl) => (
                    <button
                      key={bl.buylist_id}
                      className="flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-sm hover:bg-accent"
                      onClick={async () => {
                        // Pin to the concrete variant selected (fall back to the
                        // clicked row's variant when a filter is "all").
                        const addCondition =
                          condition === "all" ? card.sealedCondition : condition;
                        const addEdition =
                          edition === "all" ? card.variantEdition : edition;
                        await addToBuylist(
                          bl.buylist_id,
                          "pokemon_sealed",
                          card.card.card_id,
                          0,
                          null,
                          { sealedCondition: addCondition, variantEdition: addEdition }
                        );
                        setAddedTo(bl.name);
                        setTimeout(() => setAddedTo(null), 2000);
                      }}
                    >
                      {bl.name}
                    </button>
                  ))}
                </PopoverContent>
              </Popover>
            )}
            <AddToLotPopover
              mode="sealed"
              productId={card.card.card_id}
              sealedCondition={condition === "all" ? card.sealedCondition : condition}
              variantEdition={edition === "all" ? card.variantEdition : edition}
            />
          </div>
        )}
      </DialogContent>
    </Dialog>
  );
}

function ListingTable({
  listings,
  conditionHeader,
  t,
}: {
  listings: DetailListing[];
  conditionHeader: string;
  t: (key: import("@/lib/i18n").TranslationKey) => string;
}) {
  const { displayCurrency, convertPrice } = useCurrency();

  if (listings.length === 0) {
    return <p className="text-sm text-muted-foreground">{t("modal.noListings")}</p>;
  }

  return (
    <div className="rounded-md border">
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead>{t("modal.price")}</TableHead>
            <TableHead>{t("modal.location")}</TableHead>
            <TableHead>{conditionHeader}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {listings.map((l, i) => {
            let symbol = l.currencySymbol;
            let price = l.price;
            if (displayCurrency !== "none") {
              const converted = convertPrice(l.price, l.currencyCode);
              symbol = converted.symbol;
              price = converted.price;
            }
            return (
              <TableRow key={i}>
                <TableCell>
                  {symbol}
                  {price}
                </TableCell>
                <TableCell>
                  <div className="flex items-center gap-1.5">
                    {l.listingUrl ? (
                      <a
                        href={l.listingUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="underline-offset-2 hover:underline"
                        onClick={(e) => e.stopPropagation()}
                      >
                        {l.locationName}
                      </a>
                    ) : (
                      <span>{l.locationName}</span>
                    )}
                    {l.marketRegion && (
                      <Badge variant="secondary" className="h-auto px-1.5 py-px text-xs">
                        {l.marketRegion}
                      </Badge>
                    )}
                  </div>
                </TableCell>
                <TableCell>{l.conditionLabel}</TableCell>
              </TableRow>
            );
          })}
        </TableBody>
      </Table>
    </div>
  );
}
