...
export default function SealedCurationView() {
  ...
  // One-step undo (u) via G4 compensation - see CurationView for the rationale.
  ...
  const rememberDecision = useCallback((c: Candidate) => {
    setUndoState(null);
    setLastDecision({ id: c.candidate_id, label: c.product?.english_name ?? c.product?.name ?? c.ocr_cell_label_text ?? `#${c.candidate_id}` });
  }, []);
  ...
  const reject = useCallback(async (c: Candidate, notes?: string | null) => {
    if (await act(() => supabase.rpc("reject_sealed_image_buylist_candidate", { p_candidate_id: c.candidate_id, p_curator_notes: notes ?? null }))) rememberDecision(c);
  },
  // eslint-disable-next-line react-hooks/exhaustive-deps
  [rememberDecision]);
  const sendBack = useCallback((c: Candidate, notes?: string | null) =>
    ...
    void act(() => supabase.rpc("mark_sealed_image_buylist_candidate_needs_review", { p_candidate_id: c.candidate_id, p_curator_notes: notes ?? null })),
  // eslint-disable-next-line react-hooks/exhaustive-deps
  []);

  // Reverse a decision through G4 compensation (sealed twin). Idempotent by
  // request_id; surfaces `conflicted`/`failed` honestly.
  const undoDecision = useCallback(async (candidateId: number) => {
    setUndoState({ running: true });
    const { data, error } = await supabase.rpc("compensate_sealed_image_curation_decision", {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      } else if (e.key === "y" && cur?.candidate_product_id && status !== "auto_approved") {
        e.preventDefault();
        void approve(cur);
  ...
  return (
    <div className="space-y-4 p-4">
      ...
      {/* One-step undo + honest outcome (see CurationView). */}
...
export function SealedCurationCandidateCard({ c, idx, status, language, saving, selected, onSelect, onApprove, onReject, onSendBack, onUndo }: {
  ...
  // Spot-check reversal for the auto_approved tab (G4 compensation).
