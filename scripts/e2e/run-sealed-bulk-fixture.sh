...
fail() {
  echo "Sealed bulk browser acceptance error: $*" >&2
  exit 1
}
...
artifact_root="${E2E_ARTIFACT_ROOT:-/tmp/tcg-sealed-bulk-e2e-${run_token}}"
...
for _ in $(seq 1 120); do
  if curl -fsS "${app_url}/e2e/sealed-bulk" >/dev/null 2>&1; then
    ready=1
    break
  fi
...
node scripts/e2e/sealed-bulk-fixture.mjs
