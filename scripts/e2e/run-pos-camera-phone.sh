...
fail() {
  echo "POS camera phone acceptance error: $*" >&2
  exit 1
}
...
[[ -n "$api_url" && -n "$anon_key" ]] || fail "local Supabase status omitted API_URL or ANON_KEY"
...
node scripts/e2e/pos-camera-phone.mjs
