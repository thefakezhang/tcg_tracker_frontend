...
artifact_root="${E2E_ARTIFACT_ROOT:-/tmp/tcg-japan-exclusivity-e2e-${run_token}}"
...
for _ in $(seq 1 90); do
  if curl -fsS "${app_url}/e2e/japan-exclusivity" >/dev/null 2>&1; then
    ready=1
    break
  fi
...
node scripts/e2e/japan-exclusivity-fixture.mjs
