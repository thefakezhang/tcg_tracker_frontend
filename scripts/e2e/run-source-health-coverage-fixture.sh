...
fail() {
  echo "Source-health coverage browser acceptance error: $*" >&2
  exit 1
}

if [[ "${TCG_SOURCE_HEALTH_XVFB:-}" != "1" ]]; then
  command -v xvfb-run >/dev/null || fail "xvfb-run is required for headed browser evidence"
  export E2E_HEADED=1
  export TCG_SOURCE_HEALTH_XVFB=1
  exec xvfb-run -a "$0" "$@"
fi
...
[[ -x "${next_bin}" ]] || fail "Next.js is unavailable at ${next_bin}; install dependencies or set TCG_FRONTEND_DEPENDENCY_ROOT"
command -v flock >/dev/null || fail "flock is required"

exec 9>>"${lock_path}"
if ! flock -n 9; then
  echo "The shared browser lane is active; retry after its current browser or Docker gate finishes" >&2
  exit 75
fi
...
artifact_root="${E2E_ARTIFACT_ROOT:-/tmp/tcg-source-health-coverage-e2e-${run_token}}"
...
for _ in $(seq 1 120); do
  if curl -fsS "${app_url}/e2e/source-health-coverage" >/dev/null 2>&1; then
    ready=1
    break
  fi
...
node scripts/e2e/source-health-coverage-fixture.mjs
