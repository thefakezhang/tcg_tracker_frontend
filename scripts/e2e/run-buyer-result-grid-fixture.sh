...
exec 9>"${lock_path}"
if ! flock -n 9; then
  echo "The shared browser lane is active; retry when the current browser or Docker gate finishes" >&2
  exit 75
fi
...
artifact_root="${E2E_ARTIFACT_ROOT:-${frontend_root}/docs/evidence/buyer-result-grid}"
log_root="${E2E_LOG_ROOT:-/tmp/tcg-buyer-result-grid-e2e-logs}"
next_log="${log_root}/next-dev-${app_port}.log"
...
for _ in $(seq 1 90); do
  if curl -fsS "${app_url}/e2e/buyer-result-grid" >/dev/null 2>&1; then
    ready=1
    break
  fi
...
if [[ "${ready}" -ne 1 ]]; then
  ...
  echo "Controlled buyer-result fixture server did not become ready" >&2
  exit 1
fi
...
node scripts/e2e/buyer-result-grid-fixture.mjs
