...
fail() {
  echo "Pokemon variant projection browser acceptance error: $*" >&2
  exit 1
}

if [[ "${TCG_POKEMON_VARIANT_XVFB:-}" != "1" ]]; then
  command -v xvfb-run >/dev/null || fail "xvfb-run is required for headed browser evidence"
  export E2E_HEADED=1
  export TCG_POKEMON_VARIANT_XVFB=1
  exec xvfb-run -a "$0" "$@"
fi
...
artifact_root="${E2E_ARTIFACT_ROOT:-/tmp/tcg-pokemon-variant-projection-e2e-${run_token}}"
...
node scripts/e2e/pokemon-variant-projection-fixture.mjs
